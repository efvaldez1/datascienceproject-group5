...
foodsecincurb_table <- table(data2$quint, data2$areatype2, data2$q31)
foodsecincurb_temp<-as.data.frame(foodsecincurb_table)
colnames(foodsecincurb_temp)<-c("Income", "Area", "Response", "Freq")

foodsecincurb_temp <- foodsecincurb_temp %>%
  group_by(Income, Area) %>%
  mutate(Perc = 100*(Freq / sum(Freq)))

foodsecincurb_temp$Response<-factor(foodsecincurb_temp$Response)
foodsecincurb_temp$Income <- factor(foodsecincurb_temp$Income, levels = c(1:5),
                                    labels = c("Quintile 1", "Quintile 2", "Quintile 3", "Quintile 4", "Quintile 5"))

tiff('foodsec_incurb2.tiff', width = 1200, height = 699, res=100, compression = "lzw")
foodsecincurb <- foodsecincurb_temp %>% filter(Perc > 0.01) %>%
